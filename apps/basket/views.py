...
from .models import Basket, Item, Favorite, Product
from apps.basket.forms import AddToFavoriteForm, AddToCartForm
...
class AddToCartView(LoginRequiredMixin, View):
    def post(self, request, product_id):
        ...
        existing_item = Item.objects.filter(product=product, basket=cart).first()
        if existing_item:
            existing_item.quantity += quantity
            existing_item.save()
        else:
            Item.objects.create(product=product, basket=cart, quantity=quantity)
        return redirect('cart')
